...
const Navbar = () => {
  ...
  const taskboards = useSelector((state) => state.auth.user?.taskboards);
  ...
  // For permissions 1>
  const adminLinks = [
    ["Attendance", () => changePage("attendance"), DoneAllIcon],
  ];
  ...
  const userLinks = [
    [
      "Student login",
      () => {
        dispatch(uiActions.setState({ name: "isStudentLogin", value: true }));
        changePage("/login");
      },
      AccountBox
    ],
    [
      "Staff login",
      () => {
        dispatch(uiActions.setState({ name: "isStudentLogin", value: false }));
        changePage("/login");
      },
      Person
    ],
  ...
  const setAnchorElement = (el, name) => dispatch(uiActions.setState({ name: name, value: el }));
  const list = (anchor) => (
    <Box
        ...
        {isLoggedIn &&
          <ListItem key="chooseTaskboard" disablePadding>
            ...
            >
              ...
              <ListItemText primaryTypographyProps={{ color: "primary.main" }} primary="Choose taskboard" />
            </ListItemButton>
            <Menu
              id="taskboard-menu"
            ...
            >
              {(taskboards && taskboards.length > 0) ? taskboards.map((item) => (
                <Link key={item.name} to="taskboard" className='navbar-link'>
                  <MenuItem
                    onClick={() => {
                      dispatch(taskActions.setState({ stateName: "choosenTaskboardId", value: item.id }));
                      dispatch(taskActions.setState({ stateName: "latestUpdateTime", value: null }));
                      toggle("navbarToggle");
                    }}
                  >
                    <ListIcon style={theme.styles.iconPrimary} />
                ...
                <MenuItem key="noTaskboards" onClick={() => toggle("navbarToggle")}>
                  <Typography textAlign="center" sx={{ color: "primary.main", ml: 1 }}>No taskboards joined</Typography>
                </MenuItem>
              }
            </Menu>
